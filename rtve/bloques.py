#!/usr/bin/env python3
"""El catálogo de volúmenes de la oposición de RTVE.

Aquí está **lo que esta oposición tiene de suya** y `libro.py` no puede llevar
dentro: su **fecha de corte**, los **avisos** que van pegados a cada respuesta y
los **veinticinco bloques** con sus temas. El armazón del volumen —portada,
índice, maquetación, esquema detrás del cuerpo, respuestas al final— es común a
todas las oposiciones y sigue en `herramientas/libro.py`.

Se cargaba desde el propio `libro.py` mientras hubo una sola oposición. Con tres
no vale: el catálogo de Correos no tiene nada que ver con éste, y tenerlos
juntos en la herramienta obligaría a leer dos mil líneas ajenas para añadir un
bloque. Cada oposición trae el suyo en este fichero, y la herramienta lo carga
de la raíz de trabajo (`herramientas/raiz.py`).

Cuando entre otra ocupación tipo se añade una entrada a `BLOQUES` y no se toca
nada más.
"""
