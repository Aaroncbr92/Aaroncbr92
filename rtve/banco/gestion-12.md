# Gestión · Tema 12 · El proceso contable y las cuentas anuales

2 preguntas reales sacadas de los cuadernillos de octubre y noviembre de 2024.
La respuesta es la de la plantilla oficial; donde pone «sin plantilla» es que
no se pudo emparejar.

---

**15_preguntas_gestion · nº 49 · respuesta: a**

```
49.- ¿Cuáles son los libros más importantes en contabilidad?
a) El libro diario y el libro de inventario y cuentas anuales.
b) El libro de caja y el libro de bancos.
c) El libro mayor y el libro de bancos.
d) El libro de caja y el libro de entradas y salidas de almacén.
```

---

**15_preguntas_gestion · nº 101 · respuesta: c**

```
101.-Seleccione la opción FALSA. ¿Qué documentos integran las cuentas anuales?
a) La cuenta de pérdidas y ganancias.
b) La memoria.
c) El estado de cambios en el patrimonio bruto.
d) El balance.
```

