...
El mismo armazón sirve para **todos los bloques de todas las oposiciones**: se
diferencian en qué temas llevan, de qué carpeta salen y qué avisos hay que
imprimir con las respuestas, y eso está en `BLOQUES`, que **no vive aquí** sino
en el `bloques.py` de cada oposición. La herramienta lo carga de la raíz de
trabajo, y **no se duplica el código**: un volumen escrito dos veces se
...
Uso:  libro.py <bloque> [salida.html]
      ...
      Se trabaja dentro de la carpeta de la oposición, o se nombra con OPO:
      cd rtve && python3 ../herramientas/libro.py general
...
# El catálogo de la oposición: su fecha de corte y sus bloques. Vive en la
# carpeta de la oposición, en `bloques.py`, y no aquí, porque es lo único que
# cambia entre RTVE, Correos y el Auxiliar de la AGE. Lo demás de este fichero
# —cómo se arma y se maqueta un volumen— sirve para las tres sin tocar una línea.
CATALOGO = raiz.catalogo()
CORTE = CATALOGO.CORTE
