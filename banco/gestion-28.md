# Gestión · Tema 28 · Matemática financiera

4 preguntas reales sacadas de los cuadernillos de octubre y noviembre de 2024.
La respuesta es la de la plantilla oficial; donde pone «sin plantilla» es que
no se pudo emparejar.

---

**15_preguntas_gestion · nº 10 · respuesta: a**

```
10.- Se colocan 18.000 euros al 6% anual en capitalización simple durante 3 meses. ¿Cuál es
el rédito trimestral equivalente?
a) 0.015.
b) 270.
c) 0.02.
d) 360.
```

---

**15_preguntas_gestion · nº 48 · respuesta: a**

```
48.- En matemáticas financieras, ¿el Valor Actual Neto (VAN) se calcula a partir de?
a) Restar el costo inicial de la inversión al valor presente de los flujos de efectivo.
b) Sumar el costo inicial de la inversión al valor presente de los flujos de efectivo.
c) Multiplicar el costo inicial de la inversión por la tasa de rendimiento.
d) Dividir el costo inicial de la inversión por el valor presente de los flujos de efectivo.
```

---

**15_preguntas_gestion · nº 82 · respuesta: b**

```
82.- ¿Cómo calcular el descuento comercial? Siendo N: nominal, |: tipo de interés, n: tiempo
hasta el vencimiento medido en términos anuales (días/365) y Cs: comisión de servicio
referida al nominal.
a) Dc=(CsxIxn)+N
b) Dc=(N x1xn)+Cs
c) Dc=(NxCsx)/n
d) Dc=(Nxnx!l)-Cs
```

---

**15_preguntas_gestion · nº 99 · respuesta: d**

```
99.- Determine cuántos años se tiene que invertir un capital de 15.000 euros para que se
conviertan en 45.000 euros si el banco ofrece un interés del 2,5% simple semestral.
a) 80 años.
b) 24 años.
c) 35 años.
d) 40 años.
```

