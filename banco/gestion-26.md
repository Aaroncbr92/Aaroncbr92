# Gestión · Tema 26 · La gestión por competencias

2 preguntas reales sacadas de los cuadernillos de octubre y noviembre de 2024.
La respuesta es la de la plantilla oficial; donde pone «sin plantilla» es que
no se pudo emparejar.

---

**15_preguntas_gestion · nº 63 · respuesta: sin plantilla**

```
63.- El modelo 360% es un método de:
a) Evaluación de personal.
b) Motivación de personal.
c) Estratificación de personal.
d) Retribución de personal.
```

---

**15_preguntas_gestion · nº 67 · respuesta: sin plantilla**

```
67.- Para evaluar el desempeño laboral NO es correcto utilizar:
a) Desensibilización sistemática (DS).
b) Evaluación 180 grados.
c) Evaluación 270 grados.
d) Assessment center.
```

