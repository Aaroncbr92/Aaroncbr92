# Gestión · Tema 13 · El patrimonio y el balance de situación

4 preguntas reales sacadas de los cuadernillos de octubre y noviembre de 2024.
La respuesta es la de la plantilla oficial; donde pone «sin plantilla» es que
no se pudo emparejar.

---

**15_preguntas_gestion · nº 19 · respuesta: sin plantilla**

```
19.- ¿En qué parte del balance deberán figurar las existencias?
a) En el pasivo.
b) En el patrimonio neto.
c) En el activo corriente.
d) En el activo no corriente.
```

---

**15_preguntas_gestion · nº 80 · respuesta: sin plantilla**

```
80.- Las fuentes de financiación de una empresa (recursos propios y ajenos) figurarán en:
a) El activo.
b) El pasivo.
c) Pueden figurar en el activo y el pasivo.
d) No figuran en el balance.
```

---

**15_preguntas_gestion · nº 95 · respuesta: sin plantilla**

```
95.- La ordenación de las masas patrimoniales de activo, se establece en función de.
a) La liquidez.
b) La exigibilidad.
Cc) La cuantía de los bienes.
d) A criterio del personal contable.
```

---

**15_preguntas_gestion · nº 106 · respuesta: sin plantilla**

```
106.- ¿Cómo se ordenan las masas patrimoniales en el balance de situación?
a) El activo de mayor a menor liquidez y el pasivo de menor a mayor exigibilidad.
b) El activo de mayor a menor exigibilidad y el pasivo de menor a mayor liquidez.
c) El activo de menor a mayor liquidez y el pasivo de menor a mayor exigibilidad.
d) El activo de menor a mayor exigibilidad y el pasivo de menor a mayor liquidez.
```

