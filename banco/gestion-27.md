# Gestión · Tema 27 · El proceso de producción en televisión

3 preguntas reales sacadas de los cuadernillos de octubre y noviembre de 2024.
La respuesta es la de la plantilla oficial; donde pone «sin plantilla» es que
no se pudo emparejar.

---

**15_preguntas_gestion · nº 50 · respuesta: sin plantilla**

```
50.- En televisión, el equipo de producción:
a) Coordina las actividades de los equipos de realización, dirección y dirección artística y
técnica.
b) Se ocupa de la decoración del plató.
c) Vigila que se realicen las tomas de cámara necesarias.
d) Coordina la elaboración del guión.
```

---

**15_preguntas_gestion · nº 71 · respuesta: sin plantilla**

```
71.- ¿Cuál de las siguientes características es típica de un servicio VoD?
a) Programación en horarios fijos.
b) Acceso a contenido en directo únicamente.
c) Transmisión de contenido solo a través de televisión por cable.
d) Acceso a contenido multimedia bajo demanda en cualquier momento.
```

---

**15_preguntas_gestion · nº 92 · respuesta: sin plantilla**

```
92.- ¿En qué fase del proceso de producción en televisión se elabora el presupuesto?
a) Producción.
b) Preproducción.
c) Postproducción.
d) Distribución.
```

