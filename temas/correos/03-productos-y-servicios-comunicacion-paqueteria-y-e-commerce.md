# Tema 3 del específico de Correos · Productos y servicios: comunicación, paquetería y comercio electrónico

<!-- portada -->

|  |  |
| --- | --- |
| **Bloque** | Temario específico · Correos · punto 3 |
| **Sirve para** | **Correos · Grupo Profesional IV** |
| **Fuente** | **El documento de referencia de Correos.** El catálogo de productos **no está en ninguna norma**: es ficha de empresa, y cambia sin publicarse en el Boletín |
| **Identificador** | — |
| **Redacción que se estudia** | **No procede**: el catálogo no tiene redacción consolidada. Las **dos normas que el tema sí usa** van en su redacción vigente |
| **Norma de apoyo** | **Real Decreto 1829/1999** · `BOE-A-1999-24919` y **Ley 39/2015** · `BOE-A-2015-10565` |
| **Fuente principal** | El documento de referencia que publica la propia Correos para esta convocatoria, **tema 3**, reconocido ópticamente |
| **Extensión** | **8.862 palabras** |

<!-- /portada -->

Las siglas de este tema, presentadas de entrada: el franqueo en destino (**FD**); el correo comercial
de respuesta internacional (**CCRI**); la prueba de entrega electrónica (**PEE**); la entrega exclusiva
al destinatario (**EDD**); el código de entrega directa (**CED**); la Ley del Procedimiento
Administrativo Común de las Administraciones Públicas (**PACAP**), que es la Ley 39/2015; la Ley
Orgánica del Poder Judicial (**LOPJ**); el documento único aduanero (**DUA**); la Ley del Contrato de
Transporte Terrestre de Mercancías (**LTM**); el sistema de gestión de contenidos (**CMS**, por sus
siglas inglesas); el retorno de la inversión (**ROI**, por sus siglas inglesas); el Instituto Nacional
de Estadística (**INE**); la venta de empresa a empresa (**B2B**) y de empresa a particular (**B2C**);
y Correos Express (**CEX**).

> Enunciado del programa (Anexo III del Primer Desarrollo de las Bases de la convocatoria de ingreso
> de personal laboral fijo en la Sociedad Estatal Correos y Telégrafos, Grupo Profesional IV, tema 3):
> «TEMA 3. Productos y servicios: Comunicación. Paquetería de Correos y de Correos Express. Servicios
> e-commerce y Citypaq.»

**Éste es el tema más extenso del documento de referencia —ciento treinta y dos páginas— y el que más
preguntas se lleva.** **Y es un catálogo**: pesos, medidas, plazos, zonas e importes, producto por
producto. **Eso cambia la manera de comprobarlo.** En un tema de normas basta con ir al Boletín; aquí
**no hay Boletín al que ir**: cada cifra es un dato de empresa que sólo está en la página impresa.
**Este volumen ha ido a esa página cifra por cifra**, y el apartado final cuenta cuántas venían mal.

<!-- indice -->

## Índice

- [1. Cómo leer un catálogo de productos postales](#1-cómo-leer-un-catálogo-de-productos-postales)
- [2. La carta y la tarjeta postal](#2-la-carta-y-la-tarjeta-postal)
  - [La ficha de la carta ordinaria](#la-ficha-de-la-carta-ordinaria)
- [3. La carta certificada](#3-la-carta-certificada)
  - [El código de barras, y lo que dicen sus dos primeras letras](#el-código-de-barras-y-lo-que-dicen-sus-dos-primeras-letras)
  - [Los dos paquetizados: estándar y premium](#los-dos-paquetizados-estándar-y-premium)
- [4. Las notificaciones](#4-las-notificaciones)
- [5. El marketing directo](#5-el-marketing-directo)
  - [Publibuzón, Publicorreo Óptimo y Publicorreo Premium](#publibuzón-publicorreo-óptimo-y-publicorreo-premium)
  - [Los cuatro productos de datos](#los-cuatro-productos-de-datos)
- [6. Las respuestas comerciales](#6-las-respuestas-comerciales)
- [7. Libros, publicaciones periódicas y cecogramas](#7-libros-publicaciones-periódicas-y-cecogramas)
- [8. Los servicios telegráficos](#8-los-servicios-telegráficos)
- [9. La paquetería nacional de Correos](#9-la-paquetería-nacional-de-correos)
  - [La paquetería internacional](#la-paquetería-internacional)
- [10. Los valores añadidos, producto por producto](#10-los-valores-añadidos-producto-por-producto)
  - [Qué es cada valor añadido](#qué-es-cada-valor-añadido)
  - [La tabla que el documento no junta](#la-tabla-que-el-documento-no-junta)
  - [El tercer intento de las notificaciones](#el-tercer-intento-de-las-notificaciones)
- [11. Valija y los servicios del Camino de Santiago](#11-valija-y-los-servicios-del-camino-de-santiago)
- [12. Correos Express](#12-correos-express)
- [13. Los canales digitales](#13-los-canales-digitales)
  - [Las soluciones de comercio electrónico](#las-soluciones-de-comercio-electrónico)
- [14. Citypaq](#14-citypaq)
- [15. Lo que este tema no da, y dónde está](#15-lo-que-este-tema-no-da-y-dónde-está)
- [16. Trazabilidad](#16-trazabilidad)

<!-- /indice -->

## 1. Cómo leer un catálogo de productos postales

**Antes de los productos, tres convenciones que el documento da una sola vez y luego usa en las ciento
treinta páginas restantes.** **Quien no las tenga, no entiende ninguna ficha.**

| Convención | Qué significa |
|---|---|
| **D + X** | **«D» es el día de la admisión**; el número que suma son **los días de plazo para la entrega**, contados **a partir del día siguiente al de la admisión**. El propio documento lo ejemplifica: **un envío admitido el lunes 13 con plazo D+2 se entrega el miércoles 15** |
| **L, A, A, D** | **L es largo; A, ancho; A, alto; D, diámetro.** Que ancho y alto compartan inicial es del documento, y obliga a leer el orden |
| **Días hábiles** | **De lunes a viernes**, con la reforma de la Ley 39/2015: **quedan fuera sábados, domingos y festivos** |

**Y dos conceptos de la ficha que se preguntan por sí solos**: **estar en lista** es que el envío
está en una oficina a disposición del usuario, y **el medio de franqueo** son los efectos o signos que
acreditan el pago del servicio postal.

## 2. La carta y la tarjeta postal

**El documento define las dos por separado, y la diferencia se pregunta.** **Carta**, citada:

> «**todo envío cerrado cuyo contenido no se indique ni pueda conocerse, así como toda comunicación en
> forma escrita sobre soporte físico de cualquier tipo, que tenga carácter actual y personal.**»

**Tarjeta postal**, citada:

> «**toda pieza rectangular de cartulina consistente, o material similar, lleve o no el título de
> tarjeta postal, que circule al descubierto y que contenga un mensaje de carácter actual y
> personal.**»

---

**La carta va cerrada; la tarjeta circula al descubierto.** **Y también se consideran cartas** los
envíos de **recibos, facturas, documentos de negocios, estados financieros y cualesquiera otros
mensajes que no sean idénticos**: ese «que no sean idénticos» es lo que separa la carta del
publicorreo.

### La ficha de la carta ordinaria

| Campo | Dato |
|---|---|
| **Ámbito de admisión** | **Todo el territorio nacional, incluido Andorra** |
| **Ámbito de destino, internacional** | **Zona 1: Europa, incluida Groenlandia. Zona 2: resto de países, excepto los de las zonas 1 y 3. Zona 3: Australia, Canadá, Estados Unidos, Japón, Nueva Zelanda y Rusia** |
| **Plazo nacional** | **D+3 en el 93 % de los envíos** y **D+5 en el 99 %** |
| **Plazo internacional** | **Europa, de 2 a 4 días hábiles**; resto, según país |
| **Peso máximo** | **2 kg** |
| **Medidas máximas** | **Sobre o caja: L+A+A = 90 cm**, con ninguna medida por encima de **60 cm**. **Rollo o tubo: L+2D = 104 cm**, con **D ≤ 90 cm**. **Tarjeta postal: 23,5 × 12 cm** |
| **Medidas mínimas** | **Sobre o caja: 14 × 9 cm**. **Rollo o tubo: L+2D = 17 cm**, con la mayor **por encima de 10 cm**. **Tarjeta postal: 14 × 9 cm** |
| **Entrega** | **Un único intento**, en el buzón. Si no cabe y el destinatario no está, **aviso de llegada y quince días en lista** |
| **Rehúse** | **El destinatario o la persona autorizada puede rehusar el envío en el momento de la entrega, ANTES DE ABRIRLO** |
| **Garantías** | **No procede indemnización alguna**, por no ser un producto registrado |

**La zona 3 es la pregunta clásica**, porque es contraintuitiva: **Estados Unidos y Canadá están en la
zona más cara, con Australia, Japón, Nueva Zelanda y Rusia**, y **Groenlandia está en Europa**.

**Los envíos por debajo de las medidas mínimas llevan una etiqueta anexa de 10 × 7 cm** con la
dirección y el franqueo. **Ese 10 × 7 se repite en casi todos los productos del tema**, y conviene
fijarlo de una vez.

**Carta Digital** permite enviar cartas ordinarias y certificadas **desde la Oficina Virtual**:
Correos las imprime y las entrega en sobre cerrado. **El cliente elige tipo de franqueo —ordinario o
certificado—, formato —color o blanco y negro—**, y **redacta el texto en el momento o adjunta un
PDF**. **Las características del servicio son las mismas que las de una carta depositada en un buzón
de calle.**

## 3. La carta certificada

**El certificado no es un producto distinto: es una carta con un servicio añadido.** El documento lo
define así, citado:

> «**aquel que, previo pago de una cantidad predeterminada a tanto alzado, comporta una garantía fija
> contra los riesgos de pérdida, robo o deterioro, y que facilita al remitente, en su caso y a
> petición de este, una prueba de depósito del envío postal o de su entrega al destinatario.**»

---

**Peso, medidas, zonas y plazos son los mismos que los de la carta ordinaria.** **Lo que cambia es
todo lo demás**, y ahí está el examen:

| Campo | Carta ordinaria | Carta certificada |
|---|---|---|
| **Entrega** | **En el buzón** | **Bajo firma, a domicilio**, y **sólo al destinatario o a la persona autorizada** |
| **Medios de franqueo** | Pagado, metálico, sellos, tarjeta, máquina | Los mismos **más el franqueo electrónico** |
| **Garantías** | **Ninguna** | **Indemnización por retraso o demora, extravío o robo, y deterioro** |
| **Identificación** | — | **Etiqueta con código de barras adherida en el anverso**; circulan **con carácter registrado** |

**Cuántos intentos, que es lo primero que se pregunta.** **Uno.** Del documento, citado:

> «**Si el destinatario no estuviera en su domicilio, se le dejará un Aviso de Llegada para poder
> recoger la carta en su oficina postal de referencia, donde estará en lista durante 15 días.**»

---

**Un intento a domicilio y después la oficina de referencia, quince días.** **El segundo y el tercer
intento no vienen con el producto: se contratan**, y por eso el documento los pone entre los valores
añadidos. **El examen de 2023 lo preguntó** —es la número 2 del cuestionario de Reparto— y **la
respuesta es un intento**.

**Tres reglas de entrega que se preguntan mucho.** **Si el destinatario no pudiera o no supiera
firmar, firma en su lugar un testigo debidamente identificado**, y **en ningún caso puede firmar como
testigo el empleado del operador postal que efectúa la entrega**. **El destinatario puede rehusar el
envío antes de abrirlo**, igual que en la ordinaria. **Y si hay contratado un segundo o tercer
intento, no se hace** cuando el envío sea rehusado, la dirección sea incorrecta, o el destinatario
sea desconocido o haya fallecido.

**Y una regla de exportación que vale para todo el tema**: **para exportar envíos con mercancía fuera
de la Unión Europea hay que usar al menos Carta Certificada Internacional o cualquier otro producto
registrado**, porque **hay que informar telemáticamente al país de destino del contenido**.

**Su peso máximo es el mismo que el de la carta nacional: 2 kg** —es la número 102 del cuestionario
de Reparto—. **No varía según el país de destino**, que es el señuelo: **lo que varía por país son
las zonas tarifarias y los plazos, no el límite de peso de la carta.**

### El código de barras, y lo que dicen sus dos primeras letras

**Todo envío con seguimiento lleva un código de barras cuyos dos primeros caracteres identifican el
producto**, y **de todos ellos el documento sólo define uno**, porque es el que se confunde: **L3**.
Citado del glosario:

> «**L3: Prerregistro de logística inversa. Cuando el envío de ida se ha entregado por otro
> operador, o habiendo sido gestionada la ida por Correos, la vuelta se ha prerregistrado de forma
> independiente.**»

---

**L3 es logística inversa prerregistrada**, y el documento la nombra además como **la modalidad con
la que se devuelve**: «**La modalidad para las devoluciones es la logística inversa prerregistrada
(L3) mediante referencia de cliente y con embalaje.**»

**El examen de 2023 preguntó exactamente por esas dos letras** —es la número 54 del cuestionario de
Reparto— ofreciendo Paquete Premium, Paquete Estándar y carta certificada urgente como señuelos.
**Ninguno de esos tres prefijos está en el documento de referencia**, y **tampoco hace falta**: la
pregunta se contesta con la única sigla que el documento sí define.

### Los dos paquetizados: estándar y premium

**Hay una tarifa plana de carta certificada, con sobre incluido, y tiene dos modelos.** **Los dos se
distinguen por dos cifras cada uno, y el examen las cruza**: da el peso de uno con las medidas del
otro.

**Este epígrafe no cita al documento, y hay que decir por qué.** **Las dos frases que lo dicen traen
las medidas, y el reconocimiento óptico las rompe**: devuelve «162 x 224 mm» y «184 x 250 mm», con
la confusión de nueves y cuatros que recorre todo este volumen. **Las cifras que siguen se han leído
a la vista sobre la página impresa** —es la 298 del documento—, **que escribe 162 × 229 y 189 ×
250**, y **el cuadernillo del examen escribe las mismas** en la opción que su plantilla da por
buena. **Citar el volcado aquí sería citar la errata**; se da en prosa y se declara.

| Paquetizado | Peso | Sobre | Qué incluye además |
|---|---|---|---|
| **Estándar** | **50 g** | **162 × 229 mm** | — |
| **Premium** | **100 g** | **189 × 250 mm** | **PEE (Prueba de Entrega Electrónica)** y **seguro** |

**Ésa es toda la diferencia, y es toda la pregunta.** **El examen de 2023 la hizo** —es la número 56
del cuestionario de Atención al Cliente— **ofreciendo «50 g y 189 × 250» y «100 g y 189 × 250»**:
las dos mezclan el peso de un modelo con el sobre del otro. **La buena es 50 g con 162 × 229.**

**Y los dos comparten lo demás**, que esta vez sí sale limpio del volcado y se cita:

> «**Ambas opciones ofrecen máxima facilidad (basta con coger el sobre correspondiente del expositor
> de la oficina de Correos) y tienen un precio fijo.**»

## 4. Las notificaciones

**Son envíos certificados de organismos públicos**, con **prueba de entrega física o electrónica** y
**dos intentos de entrega a domicilio, según requerimiento legal**. **La contratación incluye
siempre** el aviso de recibo o la prueba de entrega electrónica **y** la gestión de entrega con
segundo intento.

**Las dos clases se distinguen por su norma y por el turno del segundo intento**, y ésa es la
pregunta:

| Clase | Norma | Segundo intento |
|---|---|---|
| **Administrativas** | **La Ley 39/2015** | **En fecha distinta a la del primero y en distinto turno**, por lo que **generalmente se gestiona en el turno de tarde** |
| **Judiciales** | **El Reglamento Postal y la Ley Orgánica del Poder Judicial** | **En fecha distinta a la del primero**, pero **puede mantenerse en el mismo turno** |

**La forma de practicarlas está en el Real Decreto 1829/1999**, el Reglamento Postal, **que sigue en
vigor en todo lo que no se oponga a la Ley 39/2015**.

| Campo | Dato |
|---|---|
| **Ámbito** | **Todo el territorio nacional, y aquí sí: no incluye Andorra** |
| **Plazo** | **D+3 en el 93 % de los envíos**; en envíos masivos, **el compromiso se fija de acuerdo con el cliente** |
| **Permanencia en lista** | **Siete días** tras el segundo intento, salvo que el cliente indique otro plazo |
| **Rótulo** | **La palabra «Notificación»** o etiqueta identificativa, y debajo, en caracteres menores, **el acto** (citación, requerimiento, resolución) y **«Expediente núm. …»** |
| **Garantías** | **Por acuerdo con el cliente en envíos masivos**; en los no masivos, **la de una carta certificada por pérdida**. Además, **penalizaciones por incumplimiento del proceso notificador** |

**Que la notificación no incluya Andorra, cuando la carta sí, es de las diferencias que se cuelan en
un test.** **Y también que la permanencia en lista sean siete días y no quince.**

**El empleado de Correos siempre debe hacer constar su firma y su número de identificación** en el
aviso de recibo o en la prueba de entrega electrónica y en el aviso de llegada. **Todas las
notificaciones tienen control y seguimiento gratuitos en la web.**

## 5. El marketing directo

**Siete productos, y la diferencia entre los tres primeros es de examen seguro.**

### Publibuzón, Publicorreo Óptimo y Publicorreo Premium

| | **Publibuzón** | **Publicorreo Óptimo** | **Publicorreo Premium** |
|---|---|---|---|
| **Qué es** | **Publicidad sin personalizar** que llega a todos los buzones, **por buzoneo** | **Envíos postales con dirección** para promocionar bienes y servicios | El formato con **entrega a domicilio** de hasta 2 kg |
| **Lleva dirección** | **No**: no figura ningún dato sobre la identidad del destinatario | **Sí**: las señas van en el objeto o en su envoltura | **Sí** |
| **Mínimo por remesa** | **100 envíos** | **100 destinatarios**, contabilizables junto a Publicorreo Internacional | — |
| **Ámbito** | **España y Andorra** | Nacional, incluido Andorra, con **Zona A y Zona B** | Igual, con **Zona A y Zona B** |
| **Plazo** | **Entre 2 y 6 días hábiles**, **4 días hábiles en el 90 %** | **4 días hábiles en el 90 % de los envíos** | **4 días hábiles en el 90 %** |
| **Peso** | **Máximo unitario 100 g** | **800 g** | **2 kg** |
| **Medidas máximas** | **23 × 33 × 2 cm** | **Formato sobre: 23 × 33 × 2 cm** | **Caja o sobre: L+A+A = 90 cm**, la mayor ≤ 60 cm; **rollo: L+2D = 104 cm** |
| **Medidas mínimas** | **9 × 14 cm** | **Formato sobre: 9 × 14 cm** | **Caja o sobre: 9 × 14 cm**; **rollo: L+2D = 17 cm** |
| **Garantías** | — | — | **Ninguna, por ser un envío ordinario** |
| **Entrega** | **Por buzoneo**: el documento no le cuenta intentos porque no los tiene | **UN ÚNICO INTENTO**, en **casillero domiciliario** | **Al domicilio**, **por depósito en el casillero** si es posible y, **si no, intentándola con el destinatario** |

**La Zona A no es «las capitales».** Es, literalmente, **envíos con destino local, capitales de
provincia, localidades con administraciones de Correos y poblaciones de más de 50.000 habitantes
especificadas por Correos**; **la Zona B es el resto**.

**Y hay una regla propia del Publicorreo Óptimo que lo distingue de todo lo demás**: **no se aceptan
envíos por debajo de las medidas mínimas aunque lleven la etiqueta de 10 × 7 cm**, porque **no podría
automatizarse su clasificación**. **En los demás productos la etiqueta salva el envío; aquí no.**

**El acondicionamiento del Publibuzón tiene tres reglas con número**: los envíos van **al descubierto
o en sobre cerrado con la indicación «Para inspección postal abrir aquí»**; **los embalajes de
plástico han de ser autorizados por los Servicios Comerciales**; y **la publicidad se agrupa en
paquetes de 25, 50 o 100 ejemplares**, con **una relación por triplicado** que diga **peso de cada
impreso, lugar de distribución y número de ejemplares por destino**.

**Los envíos normalizados al primer tramo de tarifas** son los de **20 g**, formato sobre o tarjeta,
**entre las mínimas de 14 × 9 y las máximas de 23,5 × 12 cm**, con **espesor máximo de 5 mm**.

### Los cuatro productos de datos

| Producto | Para qué sirve |
|---|---|
| **Publicorreo Plus** | **Solución paquetizada** con tres opciones: **Captación Hogares (B2C)**, **Captación Negocios (B2B)** y **Fidelización Multisoporte**, esta última con **emailing y SMS**. **Registros entre 2 y 5 días tras la firma**; **envíos entre 2 y 4 días hábiles en el 90 % de los casos**; **máximo 100 g** y **23 × 33 × 2 cm** |
| **Correos Add** | **Perfeccionar las bases de datos del cliente**, apoyándose en **más de 20.000 empleados** que recorren a diario su zona de reparto. Dos servicios: **normalización de direcciones** —depurar duplicados y homogeneizar campos— y **enriquecimiento**, que incorpora **los códigos INE y la coordenada de la dirección** |
| **Correos Data** | **Bases de datos B2B, B2C y de Códigos Postales**, en tres productos: **Básica**, **Plus** y **Capa Cartográfica** |
| **Correos Target** | **Seleccionar el público objetivo** de la campaña |

**Los plazos de entrega por complejidad son dos escalas distintas, y el examen las cruza**:

| Complejidad | **Correos Add** | **Correos Data** (B2B y B2C) |
|---|---|---|
| **Baja** | **2 días laborables** | **2 días laborables** |
| **Media** | **3 días laborables** | **4 días laborables** |
| **Alta** | **5 días laborables** | **7 días laborables** |

**En Correos Add el plazo máximo es de 5 días**; en Correos Data llega a siete. **Y la licencia de la
base de datos de Códigos Postales tiene condiciones tasadas**: **no puede cederse, sublicenciarse ni
transferirse**, **no puede usarse en internet si la aplicación es un buscador de códigos postales
igual o similar al de la web de Correos**, y **el periodo de contratación dura un año renovable por
periodos anuales**. **La Básica y la Plus admiten actualizaciones trimestrales**; **la Capa
Cartográfica se actualiza diariamente** y **la base de datos se actualiza mensualmente para quien
haya contratado actualización**.

## 6. Las respuestas comerciales

**Franqueo en destino y correo comercial de respuesta internacional** permiten **recibir respuestas de
los clientes sin coste para ellos**: pedidos, respuestas a encuestas, solicitudes de promociones.
**Las respuestas tienen categoría de cartas** y **se depositan en cualquier buzón**; **la recepción se
centraliza en un apartado de Correos de la oficina que elija el contratante**.

| Campo | Dato |
|---|---|
| **Peso, franqueo en destino** | **Hasta 2 kg** |
| **Peso, correo comercial de respuesta internacional** | **Hasta 250 g** |
| **Medidas mínimas** | **Sobre o caja: 14 × 9 cm**; **rollo o tubo: L+2D = 17 cm**; **tarjeta postal: 14 × 9 cm** |
| **Los cuatro elementos del cajetín** | **La palabra «España», la cornamusa, la indicación «Franqueo en destino» y las siglas F. D.** |
| **Medidas de referencia del cajetín** | **52 × 22 mm** o **19,8 × 22 mm** |
| **Barra vertical** | **Color azul Pantone 541 C, o negro** |
| **Espacio de indexación** | **15 mm en el borde inferior** |

**Los cuatro elementos del cajetín y el Pantone 541 C son el tipo de dato que sólo se acierta si se ha
leído la ficha.**

## 7. Libros, publicaciones periódicas y cecogramas

**El Programa Postlibris es un acuerdo, no un producto.** **Correos y las organizaciones del sector
del libro acordaron mantener en 2022 las condiciones y descuentos especiales** para el envío de libros
por correo. **Las cuatro organizaciones adheridas se preguntan por sus siglas**:

| Sigla | Organización |
|---|---|
| **FEDECALI** | **Federación Española de Cámaras del Libro** |
| **CEGAL** (siglas de la entidad) | **Confederación Española de Gremios y Asociaciones de Libreros** |
| **FANDE** (siglas de la entidad) | **Federación de Asociaciones Nacionales de Distribuidores de Ediciones** |
| **FGEE** (siglas de la entidad) | **Federación de Gremios de Editores de España** |

**El producto Libros incluye también el material fonográfico y videográfico**, y su plazo nacional es
**entre 2 y 6 días hábiles, con el 95 % en 6 días hábiles**; el internacional, **de 6 a 8 días hábiles
en Europa**. **Peso máximo, 2 kg.**

**Las publicaciones periódicas** tienen **plazo de 4 días hábiles en el 90 % de los envíos** y el
mismo peso máximo de 2 kg. **Y traen la bonificación que más se pregunta**: **las editadas por
partidos políticos y sindicatos legalmente constituidos tienen una bonificación del 40 % sobre las
tarifas vigentes**, siempre que **no contengan otra información que la propia de aquellas entidades**,
**el peso sobrepase los 100 g** y **el destino sea nacional**.

**Los envíos de publicaciones tienen mínimo de 50 ejemplares por remesa**, sumados el ámbito nacional
y el internacional, y **la admisión de envíos masivos se hace en las Unidades de Admisión Masiva que
Correos tiene en cada provincia**.

**El cecograma** se admite **en el ámbito nacional, incluido Andorra**, con **destino nacional e
internacional**, y sus medidas son las de la carta: **máximas L+A+A = 90 cm y tarjeta postal 23,5 × 12
cm; mínimas 14 × 9 cm**.

## 8. Los servicios telegráficos

**Burofax y telegrama comparten plazo y se diferencian en casi todo lo demás**:

| | **Burofax** | **Telegrama nacional** |
|---|---|---|
| **Qué transmite** | **Mensajes o documentos** | **Mensajes** |
| **Plazo** | **24 horas**: el mismo día si se admite **antes de las 13:00**, salvo **julio, agosto y septiembre**; si no, al día siguiente hábil | **El mismo, con la misma hora y los mismos tres meses de excepción** |
| **Poblaciones pequeñas** | **Se entrega al día siguiente hábil** aunque se admita a tiempo | — |
| **Tarificación** | **Por hojas** | **En grupos de 50 palabras** |
| **Canales** | **Red de oficinas** (sólo papel), **Oficina Virtual** y **canal masivo** (PDF y Word) | **Sólo oficinas**; la Oficina Virtual queda **para clientes con contrato vigente** |
| **Entrega** | **Bajo firma del destinatario** | **Bajo firma**, **en sobre cerrado** |

**Los servicios añadidos del burofax incluyen la custodia documental 120 meses**, y **la prueba de
entrega electrónica tiene tres tipologías según el tiempo en custodia**, que es dato de ficha:

| Tipología | Custodia |
|---|---|
| **PEE5** | **5 años** |
| **PEE10** | **10 años** |
| **PEE15** | **15 años** |

## 9. La paquetería nacional de Correos

**Seis productos, y la tabla siguiente es el esqueleto del epígrafe.** **Todos comparten el criterio
de peso volumétrico**: **cuando el peso no guarda relación con el tamaño se aplica (peso/volumen) /
(167 kg/m³)**, verificado en la admisión con la fórmula **largo × alto × ancho, en centímetros,
dividido entre 6.000**.

| Producto | Plazo | Peso real máximo | Dimensiones máximas |
|---|---|---|---|
| **Paq Today** | **El mismo día, entre las 15:00 y las 21:00**. Depósito en centro logístico hasta las **14:00**; recogida en tienda o almacén hasta las **13:00** | **5 kg**, con **volumétrico máximo de 8 kg** | **39 × 39 × 29 cm** |
| **Paq Premium** | **24/48 horas** según origen y destino, **en plazo garantizado** | **30 kg** | **Caja: L+A+A = 210 cm**, la mayor ≤ **120 cm**; **rollo: L = 120 cm, D = 30 cm** |
| **Paq Estándar** | **48/72 horas**, **en plazo garantizado** | **30 kg** | **Las mismas que el Premium** |
| **Paq Retorno** | **48/72 horas** | **30 kg**, con **volumétrico máximo de 60 kg** | **Las mismas**; mínima **caja de 15 × 10 cm** |
| **Paq Ligero** | **24/48 horas**, **plazo estimado y sin indemnización por incumplirlo** | **2 kg** | **35 × 23 × 2 cm**; mínima **10 × 15 cm**. **No se permite exceso de dimensiones** |
| **Paquete Azul** | **Entre 3 y 8 días de promedio** según destino | **20 kg**, con **volumétrico máximo de 50 kg** | **Caja: L+A+A = 200 cm**, la mayor ≤ **100 cm**; **rollo: L = 100 cm, D = 15 cm** |

**El Paq Today sólo tiene una zona, la Z1 provincial**, y **admite hasta 10 bultos por expedición**.
**Su entrega consta de 2 intentos y 15 días de permanencia en oficina.** **Y su peso real máximo son
5 kg** —es la número 55 del cuestionario de Reparto—, **muy por debajo de los 30 kg del Paq Premium
y del Paq Estándar**: es el producto más ligero de la tabla y por eso la cifra se cuela.

**Del Paquete Azul hay que saber que tiene una sola modalidad de entrega, y es a domicilio**, bajo
firma —es la número 64 del cuestionario de Reparto—. **No admite oficina de referencia, ni oficina
elegida, ni Citypaq**, que son precisamente las tres que el examen ofrecía sumándolas. **Lo que sí
ocurre es que, si no se entrega en ese único intento, el destinatario dispone de 15 días para
recogerlo en la oficina de referencia de su domicilio**: **eso es recogida tras un intento fallido,
no una modalidad de entrega contratable.**

**Y del reembolso de la paquetería urgente de Correos Express, el tope es otro**: **el cobro al
destinatario se hace en efectivo y el importe máximo a cobrar es de 999,99 €** —es la número 87 del
cuestionario de Reparto, sobre el Paq 14—, con **devolución del importe por transferencia bancaria
entre 4 y 6 días laborables después de la entrega**. **No confundirlo con los topes del reembolso de
Correos**, que son **499,99 €** a domicilio o apartado y **2.499,99 €** en abono en cuenta.

**Las zonas del Paq Premium y del Paq Estándar son nueve y hay que sabérselas en orden**, porque la
pregunta suele pedir una por su número:

| Zona | Ámbito |
|---|---|
| **Z1** | **Provincial** |
| **Z2** | **Provincias limítrofes**, sólo a nivel peninsular |
| **Z3** | **Península, incluido Andorra** |
| **Z4** | **Baleares, Ceuta y Melilla** |
| **Z5** | **Canarias** |
| **Z6** | **Canarias interislas** |
| **Z7** | **Origen peninsular, destino Portugal peninsular** |
| **Z8** | **Origen Baleares, Ceuta y Melilla, destino Portugal peninsular** |
| **Z9** | **Origen Canarias, destino Portugal peninsular** |

**El Paq Retorno y el Paq Ligero se quedan en las seis primeras**, porque su ámbito es **España y
Andorra** y no alcanza a Portugal.

**Dos reglas de plazo que valen para toda la paquetería**: **en envíos con destino u origen Andorra,
Canarias, Ceuta o Melilla no se computa el tiempo destinado a la tramitación aduanera**, y **el plazo
de devolución de los envíos no entregados es de 24 horas más sobre el plazo del producto**.

**Las modalidades de negociación del peso volumétrico son cuatro**, y su lógica es que **a más densidad
admitida, menor divisor**:

| Criterio | Divisor |
|---|---|
| **125 kg/m³** | **8.000** |
| **143 kg/m³** | **7.000** |
| **167 kg/m³** | **6.000** |
| **200 kg/m³** | **5.000** |

**Y la dimensión extra tiene su propio cuadro**, para Paq Premium y Paq Estándar: **el nivel estándar
admite 120 cm en una medida y 210 cm de suma**; **el nivel extra, 170 cm y 270 cm**, con sobrecoste.

### La paquetería internacional

| Producto | Peso máximo | Nota |
|---|---|---|
| **Paq Standard Internacional** | **30 kg según destino** | **Entrega a domicilio, en mano y bajo firma**; admite **apartados postales, lista de correos y lockers** en los países que lo permitan. **Mínimas: caja 14 × 9 cm** |
| **Paquete Internacional Económico** | **30 kg según destino** | **Máximas: L+A+A = 200 cm**, sin que la mayor exceda de **105 cm** |
| **Paq Premium Internacional** | — | La opción urgente |
| **Paq Light Internacional** | **2 kg** | El equivalente ligero |
| **Paq return Internacional** | **30 kg** | La devolución desde el extranjero |

## 10. Los valores añadidos, producto por producto

**Este epígrafe es el que más preguntas suelta del tema, y no está en ningún sitio del programa: el
documento reparte los valores añadidos por la ficha de cada producto, así que quien estudia producto
a producto no los compara nunca.** **La pregunta de examen es casi siempre la misma**: da cuatro
valores y pide **cuál no lleva ese producto**, o **cuáles sí**. Se reúnen aquí.

### Qué es cada valor añadido

| Valor | Qué es |
|---|---|
| **Aviso de recibo** | **Documento físico** que integra los datos del envío y **confirma su entrega** o **certifica la imposibilidad** de la misma |
| **Prueba de Entrega Electrónica (PEE)** | Lo mismo **en electrónico**, con **firma electrónica y cotejo en la web** de Correos |
| **Reembolso** | **El destinatario abona antes de la entrega** el importe que el remitente ha puesto en el envío, y **Correos se lo reembolsa después** |
| **Valor declarado** | La **cobertura por el valor de la mercancía**, envío a envío |
| **Entrega exclusiva al destinatario (EDD)** | **Sólo se entrega al destinatario, previa identificación**. **Sólo para clientes con contrato** |
| **Certificación** | **De imposición, de contenido o de entrega** |

**Y la PEE tiene tres tipologías, por el tiempo que Correos custodia el documento**, citadas del
glosario:

> «**PEE5: Prueba de Entrega Electrónica con 5 años de custodia.**» «**PEE10: Prueba de Entrega
> Electrónica con 10 años de custodia.**» «**PEE15: Prueba de Entrega Electrónica con 15 años de
> custodia.**»

---

**Cinco, diez y quince años**, y **la que el burofax premium plus incluye es la de cinco**.

### La tabla que el documento no junta

| Producto | Lleva | NO lleva |
|---|---|---|
| **Carta certificada nacional** | **Reembolso** —a domicilio, apartado o ingreso en cuenta—, **valor declarado de la mercancía**, **aviso de recibo**, **prueba de entrega electrónica**, **certificación de imposición o entrega**, **petición de devolución o de cambio de dirección** y **petición de reexpedición** | **La entrega exclusiva al destinatario**, que es de paquetería y de contrato |
| **Notificaciones** | **Retorno de información**, **recogida a domicilio**, **clasificación**, **digitalización** y **normalización de direcciones**, todos **sólo para clientes con contrato** | — |
| **Paq Estándar** | **Seguro** en tres modalidades —la de la Ley del Contrato de Transporte Terrestre de Mercancías **(LCTTM)**, para el cliente con póliza propia; la **estándar**, incluida para quien no tiene contrato; y la **especial**, pactada—, **valor declarado de paquetería hasta 6.000 €**, **PEE**, **entrega exclusiva al destinatario**, **reembolso**, **permanencia en lista a elegir** —**15 días por defecto, 30 como máximo**—, **escaneo con validación** y **escaneo de documentos** | — |
| **Paquete Azul** | **Aviso de recibo**, **reembolso** y **valor declarado** | — |
| **Paquete Internacional Económico** | el documento único administrativo **(DUA)** de exportación a terceros países, **reembolso internacional**, **valor declarado internacional** y **seguro opcional hasta 3.000 € por envío o siniestro** | **La Prueba de Entrega Electrónica** |
| **Burofax premium en oficina** | **Acuse de recibo**, **PEE**, **custodia documental 120 meses**, **copia certificada** y **certificación de contenido, entrega o imposición** | **El valor declarado**: no es un producto de mercancía |

**Tres avisos sobre esa tabla, que son tres preguntas de 2023.**

**El primero, la carta certificada.** **Lleva reembolso, valor declarado y PEE, y NO lleva entrega
exclusiva al destinatario.** **El examen ofreció tres combinaciones con EDD dentro** —es la número 7
del cuestionario de Atención al Cliente— y la buena es la única que no la mete.

**El segundo, el reembolso del Paq Estándar, que cuenta distinto según quién envíe.** Citado:

> «**Cuenta con dos modalidades para particulares (entrega en domicilio/apartado postal, máximo
> 499,99 €, y abono en cuenta, máximo 2.499,99 €) y una solo para contrato (abono en cuenta, con un
> máximo 2.499,99 €).**»

---

**Dos para particulares, UNA para contrato.** **El examen preguntó por la de contrato** —es la
número 55 de Atención al Cliente— y puso como señuelo la pareja de particulares. **Quien estudió
«dos modalidades» sin mirar para quién, falló.**

**El tercero, el burofax, que tiene cuatro nombres y se distinguen por dos cosas: por dónde se
admite y qué valores lleva dentro.** Citado:

> «**Burofax Premium Online: se interpone desde la oficina virtual.**» «**Burofax Premium: se admite
> en oficinas tanto para particulares como para clientes con contrato.**» «**Burofax Premium Plus:
> es un Burofax Premium con PEE5 y Certificación, solo para particulares.**» «**Burofax Básico Plus:
> es un Burofax Premium con PEE5 y Certificación, solo para contrato.**»

---

**Premium Plus y Básico Plus son la misma cosa con distinto destinatario**: **premium con PEE5 y
certificación**, el primero **para particulares** y el segundo **para contrato**. **El examen
preguntó qué es un premium plus** —es la número 18 del cuestionario de Reparto— y **la respuesta es
«PEE y certificación»**, no las dos horas de entrega, que no existen en ningún burofax: **el plazo
del burofax es de 24 horas**, con entrega el mismo día si se admite **antes de las 13:00**, salvo
**julio, agosto y septiembre**.

**Y una cifra del burofax que se cruza con la otra**: **la custodia de los documentos emitidos es de
5 años, 60 meses**; **la custodia documental que se contrata como valor añadido es de 120 meses**.
**Sesenta y ciento veinte, y no son lo mismo.**

### El tercer intento de las notificaciones

**El segundo intento de entrega de una notificación va incluido; el tercero, no.** El documento
coloca el **tercer intento de entrega** en la lista de **valores añadidos y servicios adicionales
«solo para clientes con contrato»** de la notificación, **junto al retorno de información, la
recogida a domicilio, la clasificación, la digitalización y la normalización de direcciones**. **Lo
que se contrata, se paga.**

**El examen de 2023 lo preguntó así** —es la número 102 del cuestionario de Atención al Cliente—,
ofreciendo el segundo intento, la entrega en domicilio y la entrega en apartado postal como
señuelos: **las tres van en el producto**; **el tercer intento es el que se añade**.

## 11. Valija y los servicios del Camino de Santiago

**La valija** es el intercambio recíproco de documentación **entre un punto y varios destinos o entre
varios puntos entre sí**, con **frecuencia y horarios establecidos** y **embalaje homologado por
Correos**, **hasta 20 kg por bolsa**. **Está orientada al correo interno de organizaciones con
delegaciones dispersas.**

**Los servicios del Camino son tres, y sus cifras se preguntan.**

| Servicio | Datos |
|---|---|
| **Paq Mochila** | **De lunes a domingo, festivos incluidos**, salvo **24, 25 y 31 de diciembre y 1 de enero**. **Recogida a partir de las 8:00** en el alojamiento; **entrega antes de las 14:30 del mismo día** en el siguiente. **Peso máximo 15 kg** y **distancia máxima por día 60 km** |
| **Paq Peregrino** | Es **un Paq Estándar con mayor periodo de almacenaje**, gestionado como **Paq Libre Servicio**. **Dos tipos: hasta 15 kg y hasta 25 kg.** Incluye **15 días de almacenamiento ampliables a 30, 45 y 75**. **Recogida y entrega en oficina**, con **plazo máximo de envío de 72 horas** |
| **Consigna Santiago** | **En la oficina principal de Correos de Santiago de Compostela**: mochilas, maletas, bastones, bicicletas |

**La temporada del Paq Mochila también se pregunta**: **temporada alta del 1 de abril al 31 de
octubre**, con nueve rutas disponibles; **temporada baja del 1 de noviembre al 1 de abril**, **sólo en
el Camino Francés de Sarria a Santiago**.

**Y su trazabilidad no va por código de barras como el resto del catálogo**: va por **código de
respuesta rápida (QR)** —es la número 80 del cuestionario de Reparto—. **El documento llama código QR al código de barras
bidimensional cuadrado que almacena los datos codificados para su lectura con un lector óptico**, y
**el Paq Mochila cuenta con un sistema de trazabilidad propio basado en esa lectura**: **el QR de
seguimiento va en la etiqueta con los datos de la reserva**, que **el peregrino coloca en su
equipaje**; **al recogerlo, el cartero lee el código y el peregrino recibe un correo electrónico
avisándole**, y **el cartero repite la operación al entregar**.

**Y el envío de bicicleta tiene su propia ficha**: **embalaje de 150 × 30 × 40 cm**, que **permite
introducir la bicicleta sin apearla**, con **peso máximo de 20 kg** y **plazo estimado de 3 días**.

## 12. Correos Express

**Seis productos nacionales, y todos comparten dos cifras**: **peso real máximo de 40 kg** y **criterio
volumétrico de 167 kg/m³ para envíos terrestres y 333 kg/m³ para marítimos**. **Ese 333 es exclusivo
de Correos Express y no aparece en la paquetería de Correos.**

| Producto | Qué garantiza | Ámbito |
|---|---|---|
| **Paq 10** | **Entrega antes de las 10:00** del día hábil siguiente —**10:00 a 12:00 según código postal de destino**— | **España (capitales de provincia y más de 500 poblaciones) y Portugal (principales distritos)**. **No disponible con origen Portugal** |
| **Paq 14** | **Entrega antes de las 14:00** del día hábil siguiente | **Todo el territorio nacional y todos los distritos de Portugal**. **No disponible para Baleares y Canarias con origen Portugal** |
| **Paq 24** | **Entrega el día laborable siguiente** | **España, Portugal, Ceuta, Melilla, Andorra y Gibraltar**. **D+1 peninsular, D+2 en Ceuta, Melilla, Andorra y Gibraltar** |
| **ePaq 24** | Lo mismo, pero **B2C, con destino particulares** | **Todo el territorio peninsular, España y Portugal** |
| **Paq Empresa 14** | Lo mismo que el Paq 14, pero **B2B, con origen y destino empresas** | **Territorio peninsular y la misma isla** |
| **Entrega Plus** | **Logística integral express** con tratamiento adicional bajo operativa definida | **España, Portugal, Ceuta, Melilla y Andorra** |

**El exceso de peso tiene precio en el Paq 10**: **suplemento adicional de 25 euros** en los bultos que
superen el máximo permitido.

**Los servicios a islas son tres y se distinguen por el plazo**:

| Servicio | Plazo |
|---|---|
| **Islas Express** | **D+2 en Baleares, Gran Canaria y Tenerife**; **D+3 en el resto de Canarias**; **D+3 a D+4 en Azores y Madeira**, y **D+4 en las islas menores** |
| **Islas Marítimo** | **Entre 4 y 10 días** desde el día siguiente laborable al de la admisión. **La gestión aduanera puede retrasar el servicio** |
| **Islas Docs** | **D+1 a D+2 en Tenerife y Gran Canaria**; **D+2 a D+3 en el resto de Canarias**. Su rasgo propio es el **trámite aduanero realizado antes de la salida** |

**Aquí hay una discrepancia del propio documento que conviene conocer.** **El texto que presenta Islas
Express dice «entrega en el siguiente día laborable al de su admisión, en Gran Canaria y Tenerife»**,
y **su ficha, tres renglones más abajo, dice D+2 para esas mismas islas**. **No es defecto del
volcado: las dos frases están en la página**, comprobado a la vista. **Manda la ficha, que es donde el
documento pone los plazos de todos los productos.**

**Y los dos productos internacionales de Correos Express comparten plazo**: **de D+2 a D+6 según
destino y tipología**, **el Estándar con cobertura europea** y **el Express con cobertura mundial**.

## 13. Los canales digitales

**Antes de la lista, la causa, porque el examen la pregunta suelta.** **Lo que ha hecho crecer la
paquetería es el comercio electrónico**, y el documento lo dice sin rodeos al abrir la distribución:

> «**El auge del comercio electrónico ha convertido la distribución en una herramienta fundamental
> para satisfacer las expectativas de nuestros clientes, los cuales son cada vez más exigentes y
> esperan entregas con un servicio excelente y a un precio competitivo.**»

---

**Y en el otro platillo, la caída de la carta**, por el mismo movimiento y en sentido contrario,
citado:

> «**Sin embargo, la Administración electrónica, la banca digital y las facturas electrónicas,
> especialmente durante estos dos últimos años de pandemia, han sustituido las comunicaciones en
> formato papel y, por tanto, han abandonado los servicios postales a favor de los canales
> electrónicos.**»

---

**Las dos frases son la misma historia**: **lo electrónico se lleva la carta y trae el paquete.**
**El examen de 2023 preguntó por la mitad de arriba** —es la número 32 del cuestionario de Atención
al Cliente— y ofrecía «Internet» y «la sustitución electrónica» como señuelos: **Internet es el
medio y la sustitución electrónica es lo que le pasa a la carta**; **lo que hace crecer la
paquetería es el comercio electrónico**.

| Canal | Qué es |
|---|---|
| **Mi Oficina** | **El área privada del cliente**, en **mioficina.correos.es**. Distingue **particulares** —sin contrato— de **clientes con contrato o comerciales** |
| **App Correos** | Permite **localizar un envío, localizar oficinas, Citypaq o buzones, trámites aduaneros, enviar un paquete, Paqbook —el monedero de los envíos— y el formulario en línea**. **Se puede usar sin registro, con menos funcionalidades** |
| **correos.com** | **La web institucional del Grupo**, con **siete grandes apartados**. Entre ellos, **Grupo Correos**, con las cifras clave y **la historia de más de 300 años**, y **Sala de prensa**, que identifica a los responsables de comunicación **de cada una de las siete zonas en que Correos organiza su gestión** |
| **correos.es** | **La web comercial** |
| **Correos Market** | **La plataforma de comercio electrónico**, en **market.correos.es** |

**De «Mi Oficina» se preguntan dos cosas: para quién es y qué se hace desde ella.** Citado:

> «**Desde Mi Oficina se centralizan las acciones que hace el cliente de Correos tanto a nivel
> digital como físico. Busca ofrecer a todos nuestros clientes, tanto particulares como de contrato,
> la gestión con Correos online.**»

---

**Para los dos: particulares y clientes con contrato.** **Particulares son los que no tienen ningún
contrato con Correos; clientes con contrato o comerciales, los que tienen relación contractual** y
pueden **contratar productos exclusivos**. **El examen de 2023 preguntó exactamente por ese par**
—es la número 36 del cuestionario de Reparto—.

**Y lo que se hace desde ella va más allá de enviar**: **el cliente con contrato activo vinculado ve
los productos asociados a su contrato**, y **desde la misma página se accede a las soluciones de
temperatura controlada «Correos Frío»** y **a la compra de material de envío en la Tienda de Correos
dentro de Correos Market**. **El examen lo preguntó así** —es la número 58 de Atención al Cliente—,
citando las facturas del contrato y Correos Frío: **las dos cosas están en Mi Oficina**.

**De correos.es, la web comercial, lo que se pregunta es a quién se dirige.** Citado:

> «**La página web https://www.correos.es presenta los servicios que ofrece Correos, diferenciando
> entre particulares y empresas.**»

---

**Particulares y empresas, y ésa es la respuesta** —es la número 63 de Atención al Cliente—. **No
confundirla con correos.com**, que es **la web institucional del Grupo**.

**Y Correos Market tiene además una definición de intención, que el tema 2 del documento da y el
examen pregunta por su letra.** Citada:

> «**Correos Market. Es la iniciativa estratégica de nuestra compañía para dar visibilidad y acceso a
> los emprendedores y pequeños productores de todas las localidades y pueblos de España al gran
> mercado nacional, con él contribuimos al reto demográfico.**»

---

**Visibilidad a emprendedores y pequeños productores de todos los pueblos** —es la número 99 del
cuestionario de Reparto—, y **con ello Correos dice contribuir a frenar la despoblación rural**.
**Frente a los tres señuelos —Correos Cash, Citypaq en espacios públicos y las tarjetas prepago—, es
la única que se declara iniciativa estratégica de comercio.**

**De Correos Market hay que retener el modelo económico, porque es pregunta de cifra**: **el vendedor
formaliza un contrato de paquetería sin permanencia ni compromiso de ventas mínimo**; **Correos cobra
el 100 % de las ventas y a los 30 días liquida con el vendedor**, descontando **comisiones que oscilan
entre el 3 y el 15 % del importe de la venta según el tipo de producto**.

### Las soluciones de comercio electrónico

**La tienda en línea tiene tres versiones, y sus límites de catálogo se preguntan**:

| Versión | Qué permite |
|---|---|
| **Gratuita** | **Hasta 10 productos**, bajo el dominio **mitienda.mycorreosecommerce.com** |
| **Basic** | **Hasta 200 productos**, con **gestión aduanera** y **dominio propio**, por una cuota mensual reducida |
| **Premium** | **Número ilimitado de productos**, **multilingüe** y **con distintas divisas** |

**Local E-commerce** es el servicio para **empresas, organismos públicos o asociaciones que quieran
organizar un marketplace de comercios locales**. **Y los módulos e-commerce** son piezas de software
que Correos ha diseñado **para los principales proveedores de gestores de contenidos del mercado
—Woocommerce, Magento y Prestashop—** y que **automatizan prerregistro, etiquetado, solicitud de
recogida y devolución**. **Están a disposición gratuita** de las tiendas creadas con esos gestores.

## 14. Citypaq

**Citypaq son taquillas inteligentes** para **recoger, enviar o devolver paquetes**, situadas **cerca
del domicilio, del trabajo o en lugares de tránsito** como supermercados, gasolineras o centros
comerciales.

**El código que identifica al usuario y al dispositivo se llama Código de Entrega Directa**, es
**alfabético**, **único y diferente para cada código asociado**, y **es la dirección que se facilita
para comprar en cualquier tienda no asociada dentro de los territorios permitidos**.

**Las taquillas se dividen en privadas —de acceso restringido— y públicas**: **el dispositivo privado
pide un código situado en la parte superior derecha de la pantalla de la taquilla**; **el público sólo
hay que localizarlo en el mapa, seleccionarlo y añadirlo a favoritos**.

**Los dos modelos tienen medidas distintas, y ésa es la pregunta que separa a quien ha leído la ficha
de quien no**:

| | **Modelo Reducido** | **Modelo Estándar** |
|---|---|---|
| **Huecos** | **De 6 a 30** | **Más de 30** |
| **Tamaños** | **Cuatro** | **Cinco** |
| **XS** (la talla extra pequeña, que sólo tiene el estándar) | — | **85 × 190 × 610** |
| **S** | **100 × 240 × 400** | **85 × 440 × 610** |
| **M** | **100 × 490 × 400** | **180 × 440 × 610** |
| **L** | **200 × 490 × 400** | **370 × 440 × 610** |
| **XL** | **600 × 490 × 400** | **750 × 440 × 610** |

**Las medidas van en milímetros y en el orden alto × ancho × profundo**, que el documento declara
expresamente. **El modelo estándar admite de 3 a 22 huecos por módulo**, con **configuración mínima de
1 módulo de control más 2 de expansión** y **máxima de 1 de control más 10 de expansión**, **sólo en
interior**, con **conexión monofásica de 230 V ± 10 %** y **conectividad por router móvil 4G o fibra
óptica**.

**Que el modelo reducido tenga cuatro tamaños y el estándar cinco —porque el estándar añade el XS— es
exactamente el tipo de dato que decide una pregunta.**

## 15. Lo que este tema no da, y dónde está

| Materia | Dónde está | Estado |
|---|---|---|
| **Las tarifas** de cada producto | El tarifario vigente de Correos | **NO volcadas.** El documento no las da y **cambian todos los años**: estudiarlas de un temario es estudiar un dato caducado |
| **El Reglamento Postal** completo | Real Decreto 1829/1999 | **Volcado** para este temario desde el **texto consolidado** `BOE-A-1999-24919`. Se nombra lo que el documento invoca |
| **La Ley 39/2015** | Boletín Oficial del Estado | **Volcada** para este temario desde el **texto consolidado** `BOE-A-2015-10565` |
| **La Ley Orgánica del Poder Judicial** | Boletín Oficial del Estado | **NO volcada**: se nombra como norma de las notificaciones judiciales, que es lo que el programa pide |
| **La Ley 15/2009**, del contrato de transporte terrestre | Boletín Oficial del Estado | **NO volcada**: fija el límite de responsabilidad al que el documento remite |
| **La matriz de plazos** a Canarias, Ceuta y Melilla | Un enlace del documento a la web de Correos | **NO volcada**: el documento no la reproduce, remite a ella |
| **El cuadro de sobrecostes** por dimensión extra | La ficha del Paq Premium | **Volcado en su estructura**, no en sus importes, que son tarifa |
| **Los servicios e-commerce de Correos Express** | El epígrafe 3.3.2 del documento | **Volcado**: se desarrollan con los productos de Correos Express |

## 16. Trazabilidad

| Regla del método | Fuente | Qué se ha citado |
|---|---|---|
| **Documento de referencia de Correos** | Tema 3 del documento que la empresa publicó para esta convocatoria, reconocido ópticamente página a página | **Las definiciones de carta, tarjeta postal y envío certificado; las fichas completas de carta ordinaria, carta certificada y notificaciones; los siete productos de marketing directo con sus pesos, medidas y plazos; las respuestas comerciales y su cajetín; el Programa Postlibris y sus cuatro organizaciones; las publicaciones periódicas y su bonificación; el cecograma; burofax y telegrama con sus tres tipologías de prueba de entrega electrónica; los seis productos de paquetería nacional y los cinco internacionales; las nueve zonas; los cuatro criterios de peso volumétrico; la valija y los tres servicios del Camino; los seis productos de Correos Express y los tres de islas; los cinco canales digitales; las tres versiones de tienda en línea; y los dos modelos de Citypaq con sus tamaños**, citados literalmente |

**Sobre la comprobación de este tema hay que decir algo que no vale para ningún otro.**

**Las dos lentes que contrastan contra norma no tienen aquí dónde anclar.** **Un catálogo de productos
no se comprueba contra el Boletín**: los pesos, las medidas y los plazos **no los fija ninguna norma,
los fija la empresa**. **Lo que comprueba este tema es la lente de citas**, que contrasta cada tramo
entrecomillado contra el volcado, **y la de documento**, que persigue cada cifra en negrita hasta la
fuente. **Y por encima de las dos, la comprobación a la vista sobre la página impresa**, que en este
tema ha sido el trabajo principal.

**Por qué, y es la advertencia central del volumen.** **El documento va sin capa de texto y se ha
reconocido ópticamente**, y **su tipografía dibuja el nueve de manera que el reconocedor lo lee como
un cuatro**. **En un tema de normas eso se caza solo**: un año 1447 no existe, una Ley 94/2017
tampoco. **En un catálogo no se caza nada**: **«43 %» es un porcentaje posible**, **«14 × 4 cm» una
medida posible**, **«494,44 €» un importe posible**. **La fuente corrupta validaría la copia corrupta
sin que ninguna lente dijera nada.**

**Por eso este tema ha necesitado una herramienta nueva, `correos_gemelas.py`.** **Aprovecha que un
catálogo repite los mismos campos decenas de veces**: busca **parejas de cifras que se diferencian
sólo en que un cuatro de una es un nueve en la otra**, y **pone delante la menos frecuente**. **No
prueba nada** —«24» y «29» son dos números legítimos— **pero reduce el viaje a la página impresa a las
pocas que lo merecen.**

**Lo que sacó, y todo se comprobó a la vista antes de corregirlo:**

| El volcado decía | La página imprime | Dónde |
|---|---|---|
| **D+3 en el 43 % de los envíos** | **93 %** | Página 4 |
| **14 × 4 cm** de mínimo, catorce veces | **14 × 9 cm** | Páginas 5, 58 y 60 |
| **D ≤ 490 cm** en el rollo | **D ≤ 90 cm** | Página 5 |
| **4 días hábiles en el 40 %** | **90 %** | Página 14 |
| **Formato sobre: 4 × 14 cm** | **9 × 14 cm** | Página 14 |
| **El 945 % de los envíos** | **95 %** | Página 26 |
| **39 × 349 × 24 cm** | **39 × 39 × 29 cm** | Página 41 |
| **Z4, origen Canarias destino Portugal** | **Z9** | Página 48 |
| **449,44 €, 444,94 €, 494,44 €, 494,49 € y q94,94 €**, en cinco páginas | **499,99 €** y **999,99 €** | Páginas 51, 59, 75, 79 y 89 |
| **2.444,99 € y 2.494,99 €** | **2.499,99 €** | Páginas 51 y 59 |
| **M: 100 × 4490 × 400** | **100 × 490 × 400** | Página 128 |

**Y las que la herramienta señaló y no eran defecto, que enseñan tanto como las otras**: **la
bonificación del 40 % a las publicaciones de partidos y sindicatos es 40**; **el peso real máximo de
40 kg de Correos Express es 40**; y **las medidas 440 × 610 del Citypaq estándar no son una errata de
las 490 × 400 del reducido: son dos modelos distintos**. **Las tres se miraron en su página antes de
descartarlas.**

**Ninguna cifra de este tema viene del reconocimiento sin haber pasado por la página impresa.**
